...
const SeatSelectionScreen = () => {
  ...
  useEffect(() => {
    ...
    if (route.params?.busDetails) {
      setBusDetails(route.params.busDetails);
      generateSeatLayout(route.params.busDetails.seatLayout);
      setPassengerCount(route.params.passengerCount || 1);
    } else if (selectedBus) {
      setBusDetails(selectedBus);
      generateSeatLayout(selectedBus.seatLayout || generateDefaultSeatLayout());
  ...
  const generateDefaultSeatLayout = () => {
    // Default layout: 10 rows, 4 seats per row
    const layout = [];
    ...
    rows.forEach((row, rowIndex) => {
      ...
      for (let col = 1; col <= 4; col++) {
        ...
        rowSeats.push({
          ...
          row: row,
          column: col,
          type: 'normal', // 'normal', 'premium', 'disabled'
          price: selectedBus?.price || 150000,
          isAvailable: Math.random() > 0.3, // Random availability for demo
          isSelected: selectedSeats.some(s => s.number === seatNumber),
        });
      }
  ...
  const generateSeatLayout = (layoutConfig) => {
  ...
  const handleSeatSelect = (seat) => {
    ...
    if (!seat.isSelected && selectedSeats.length >= passengerCount) {
      Alert.alert(
        'Maksimum Kursi',
  ...
  const getSeatTypeColor = (type, isAvailable, isSelected) => {
    if (!isAvailable) return '#E0E0E0';
  ...
  const renderDriverSection = () => (
    <View style={styles.driverSection}>
      <View style={styles.driverIcon}>
        <Ionicons name="steering" size={40} color="#666" />
      </View>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.busInfoCard}>
        <View style={styles.busInfoRow}>
          <Text style={styles.busName}>{busDetails?.name || 'Bus'}</Text>
          <Text style={styles.busClass}>{busDetails?.class || 'Executive'}</Text>
        </View>
        <Text style={styles.routeText}>
          {busDetails?.departure || 'Jakarta'} → {busDetails?.destination || 'Bandung'}
        </Text>
        <Text style={styles.dateText}>
          {busDetails?.departureDate ? 
            new Date(busDetails.departureDate).toLocaleDateString('id-ID') : 
            ...
            {selectedSeats.map((seat, index) => (
              <View key={index} style={styles.selectedSeatBadge}>
                ...
                <Text style={styles.selectedSeatPrice}>
                  Rp {seat.price?.toLocaleString('id-ID') || '150.000'}
                </Text>
              </View>
            ))}
          </View>
          <Text style={styles.totalText}>
            Total: Rp {selectedSeats.reduce((sum, seat) => sum + (seat.price || 0), 0).toLocaleString('id-ID')}
          </Text>
        </View>
      )}
        ...
        <View style={styles.seatLayoutContainer}>
          ...
          {seatLayout.map((row, rowIndex) => (
            <View key={rowIndex} style={styles.seatRow}>
              <Text style={styles.rowLabel}>{row[0]?.row || 'A'}</Text>
              ...
              <View style={styles.seatsInRow}>
                {row.map((seat, seatIndex) => (
                  ...
                  >
                    <Text style={[
                      styles.seatText,
                      (!seat.isAvailable || seat.isSelected) && { color: '#FFF' }
                    ]}>
                      {seat.number}
                    </Text>
                    {seat.type === 'premium' && seat.isAvailable && (
                      <Ionicons 
                        name="star" 
                        size={12} 
                        color="#FFF" 
      ...
      <View style={styles.actionContainer}>
        <View style={styles.priceSummary}>
          ...
          <Text style={styles.priceValue}>
            Rp {selectedSeats.reduce((sum, seat) => sum + (seat.price || 0), 0).toLocaleString('id-ID')}
          </Text>
        </View>
